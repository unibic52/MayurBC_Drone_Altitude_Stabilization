...
```
📁 repository
├── drone_pid_simulation.m    # Main MATLAB simulation script
├── README.md                 # This file
├── figures/
│   ├── open_loop.png         # Open-loop step response
│   ├── closed_loop.png       # Closed-loop PID step response
│   └── disturbance.png       # Disturbance response plot
...
**Why PID?**
PID is the industry standard for this class of second-order systems. The three terms directly address all three problem objectives — speed (Kp), accuracy (Ki), and stability (Kd).

**Why does the drone recover after wind?**
The integral term (Ki) continuously accumulates the error caused by the wind disturbance. Even when the wind pushes the drone down, Ki keeps building up a corrective signal until the drone is back at exactly 1m.

**Why is Kd important?**
Without Kd, the drone would overshoot the target significantly before settling. The derivative term senses how fast the error is changing and applies a braking force — reducing overshoot from ~30% down to 3.35%.
